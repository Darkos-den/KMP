    ...
    private suspend fun validate(field: Field): Boolean {
        return when (field.type) {
            ...
            is ValidationFieldType.MinSymbols -> {
                minSymbolValidate(field.value, field.type.minCount)
            }
            is ValidationFieldType.MaxSymbols -> {
                maxSymbolValidate(field.value, field.type.maxCount)
            }
            is ValidationFieldType.IntervalSymbols -> {
                intervalSymbolValidate(field.value, field.type.minCount, field.type.maxCount)
            }
